...
#ifndef ACCELSIM_PAGEWALK_H
#define ACCELSIM_PAGEWALK_H
...
class PageWalkCache {
     ...
     * pumped per cycle.
     * The ports are per internal page walk cache; i.e., if there are two internal
...
private:
     ...
     * The PageWalker::probe_pw_cache() method is used to check if a particular
...
class PageWalkQueue {
     ...
     * The actual implementation of queues and the policies regarding their
     * servicing are abstract away from the TLB and page walkers.
...
private:
    ...
    unsigned size;  // page walk queue size
    const memory_config *config;
    memory_stats_t *mem_stat;
    PageWalkSubsystem *page_walk_subsystem;
    enum
